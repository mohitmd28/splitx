<div class="mb-4.5">
    <div>
        ...
        <input type="text" name="name" placeholder="Enter name" value="<?= old('name', $group ? $group->name : '') ?>" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary" />
...
<div class="mb-4.5">
    <label class="mb-2.5 block text-black dark:text-white">
        Members <span class="text-meta-1">*</span>
    </label>
    <input type="text" placeholder="Enter the code of user and press enter to add user" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary" style="text-transform: uppercase" @keyup.enter.stop="
                                fetch(`/groups/search-user?code=${$el.value}`)
                                    .then(response => response.json())
                                    .then(data => {
                                        if(!data){
                                            alert('User not found with given code');
                                            return;
                                        }

                                        if(typeof members.find(member => member.code === data.code) === 'undefined')
                                            members.push(data);

                                        $el.value = '';
                                    })
                                    .catch(error => alert('😟 !Opps, There was an error in fetching user'));
                            " />

    <?php if (error('members')) : ?>
        <span class="danger" style="color: red;">
            <?= error('members') ?>
        </span>
    <?php endif; ?>
                ...
                <li class="flex flex-row">
                    <div class="select-none cursor-pointer hover:bg-gray-50 flex flex-1 items-center p-3">
                        <div class="flex-1 pl-1">
                            #<span class="font-medium dark:text-white" style="text-transform: uppercase;" x-text="member.code"></span>: &nbsp;
                            <span class="font-medium dark:text-white" x-text="member.first_name"></span>
                            <input type="hidden" name="members[]" :value="member.id" />
                        </div>
                        <div class="flex flex-row justify-center">
                            <button class="hover:text-primary" type="button" @click="members = members.filter(m => m.code !== member.code)">
