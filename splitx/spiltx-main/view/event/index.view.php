...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
    <h2 class="font-semibold text-title-md2 text-black dark:text-white">All Events</h2>

    <nav>
        <ol class="flex items-center gap-2">
            <li><a href="/dashboard">Dashboard /</a></li>
            <li class="text-primary">All Events</li>
        </ol>
    </nav>
</div>
<!-- Breadcrumb End -->
        ...
        <div class="max-w-full overflow-x-auto">
                    ...
                    <?php else : ?>
                        <?php foreach ($events as $event) : ?>
                            <tr>
                                ...
                                <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    <a href="#" class="inline-flex items-center justify-center rounded-full bg-primary py-2 px-8 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-5">
                                        $ Expenses
                                    </a>
                                </td>
                                <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    <div class="flex items-center space-x-3.5">
                                        <a href="/events/edit?id=<?= $event->id ?>" class="hover:text-primary">
                                        ...
                                        <button class="hover:text-primary" @click="
                                            if(confirm('Do you really want to delete?')){
                                                $refs.eventToDelete.value = '<?= $event->id ?>';
                                                $refs.eventDeleteForm.submit();
                                            }
                                        ">
            ...
            <ul class="flex justify-center gap-3 mb-2 pt-6">
                <?php for ($i = 1; $i <= $pages; $i++) : ?>
                    <li><a class="border hover:text-primary p-1 rounded" href="/events?page=<?= $i ?>"><?= $i ?></a></li>
                <?php endfor; ?>
            </ul>

            <form method="post" x-ref="eventDeleteForm" action="/events/delete" style="display: none;">
                <input type="hidden" name="id" x-ref="eventToDelete">
            </form>
        </div>
    </div>
</div>
